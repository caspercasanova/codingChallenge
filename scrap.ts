...
const copeAndInvertImageData = (
  ogBuffer: Buffer,
  negativeBuffer: Buffer
) => {};
...
const convertImage = async () => {
  let ogBuffer = await getImageBuffer();

  if (ogBuffer.toString('utf-8', 0, 2) != 'BM') {
    throw new Error('The File Format Must Be in a .BMP');
    return null;
  }

  //  const bufferHeader = readBufferHeader(ogBuffer);

  let negativeBuffer: Buffer = Buffer.alloc(ogBuffer.length);

  copyBufferHeader(ogBuffer, negativeBuffer);

  console.log(
    negativeBuffer,
    ogBuffer[ogBuffer.length - 1],
    negativeBuffer[negativeBuffer.length - 1]
  );
  // manipulateBuffer(negativeBuffer);
  // createImage(negativeBuffer);

  // https://discourse.nodered.org/t/how-can-i-load-image-file-into-the-rgb-array/16670/8
};
...
// const mutateBuffer = (buffer: Buffer) => {
//   const start = 54;
//   const finish = 1077;
//   var num = 1;
